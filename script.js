...
function checkDataForm(inputID) {
  const inputToCheck = document.getElementById(inputID);

  const validityInput = inputToCheck.validity;

  if (validityInput.rangeUnderflow) {
    inputToCheck.setCustomValidity('Cannot be less than 5 pages.');
  } else {
    inputToCheck.setCustomValidity('');
  }

  return inputToCheck.reportValidity();
}
